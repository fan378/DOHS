...
def get_token_len(wenshu_str):
    ...
    # model = AutoModelForCausalLM.from_pretrained(model_path,torch_dtype=torch.float16, trust_remote_code=True,device='cuda:0')
    # model = model.eval()
    token_length = tokenizer.build_chat_input(wenshu_str, history=[], role='user')['input_ids'].size()
    return token_length[1]

def build_data(ori_index,ori_data,data_maps,data_lengths,keshi,new_zylsh):
    ...
    ** 入院时间返回3个值 **
    ...
    for key,value in data_maps.items():
        ...
        for wenshu_index,wenshu in enumerate(wenshu_list):
            ...
            if get_token_len(temp_str) > 7800:
                break
            wenshu_str = temp_str
        # print(get_token_len(wenshu_str))
        wenshu_str = wenshu_str.strip()
        ...
        for source_key in all_sources:
            ...
            if get_token_len(input_temp) > 7800:
                break
            input_str =  input_temp
        # print(get_token_len(input_str))    
...
def get_instructions_v2024_0324(data_dir,out_dir,keshi,new_zylsh):
    if out_dir != '':
    ...
    for i in tqdm(range(datas.shape[0])):
        res_data = build_data(i,datas.iloc[i,:].copy(),data_maps,data_lengths,keshi,new_zylsh)
        # break
        if res_data == None:
            continue
