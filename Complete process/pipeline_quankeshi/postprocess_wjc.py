...
def load_model(model_path,gpu):
    ...
    model = AutoModel.from_pretrained(model_path,torch_dtype=torch.float16, trust_remote_code=True,device=gpu)
    model = model.eval()
    print('模型加载成功！')
...
def get_source_wenshu_list(zylsh, keshi):
    processed_file = os.path.join(process_dir,keshi,zylsh,'new_最终处理并合并后数据.csv')
...
def correct_bingshi(wenshu_list,ori_datas):
    suifang_keshi_dict = {}
    with open('出院小结及子字段/病史随访科室对应.csv', 'r', encoding='utf-8') as file:
...
if __name__ == '__main__':
    ...
    #psh 加载模型
    postprocess_model,postprocess_tokenizer = load_model(model_path,gpu)
