...
tuomin_fixes = []
with open('./出院小结及子字段/脱敏修复.txt','r') as f:
    for line in f.readlines():
        line = line.strip()
        tuomin_fixes.append(line.split('-->'))

transfer_keys = []
with open('./出院小结及子字段/transfer_keys.txt','r') as f:
    for line in f.readlines():
        line = line.strip()
        transfer_keys.append(line)

keshi_list = cons_keshis
def process_and_merge(data_dir,out_dir,doctor_dir):
    # ------------------------------------1.处理并合并---------------------------------------
    random.seed(2023)
    # 0:部分 1:全量
    data_type = 1
    ...
    for i in range(merged_df.shape[0]):
        ...
        if doctor_dir != '':
            with open(os.path.join(doctor_dir,f'{zylsh}.json'),'w',encoding='utf8') as f:
                json.dump(cyxj,f,ensure_ascii=False,indent=4)
...
if __name__ == '__main__':
    ...
    for zylsh in zylshs:
        ...
        now_doctor_dir = os.path.join(doctor_dir,keshi,zylsh)
        merged_df,json_datas,cyxjs = process_and_merge(now_data_dir,now_out_dir,now_doctor_dir)
