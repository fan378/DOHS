...
def get_all_wenshu_keys_to_str(keshi, ins_name, wenshu_list, need_wenshus_common_keys_dict):
    ...
    if keshi == "小儿科":
        if ins_name == "患者基本信息":
            ...
            if "入院告知书" in need_wenshus:
                res_str = transfer_wenshu_to_str(wenshu_list["入院告知书"], ["患者信息"]) # 由于["患者信息"]字段明确，且入院告知书只有一个字段，也可need_wenshus_common_keys_dict["入院告知书"]
            elif "告未成年患者监护人书" in need_wenshus:
                res_str = transfer_wenshu_to_str(wenshu_list["告未成年患者监护人书"], ["患者信息"])
            elif "患者授权委托书" in need_wenshus:
                res_str = transfer_wenshu_to_str(wenshu_list["患者授权委托书"], ["患者信息"])
            
            # 获取体征信息
            if "新入院评估单" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_list["新入院评估单"], ["三、生理状况评估"]) # 由于["患者信息"]字段明确，且入院告知书只有一个字段，也可need_wenshus_common_keys_dict["入院告知书"]
            elif "入院记录" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_list["入院记录"], ["体格检查"])
                
            # 获取诊断信息
            for need_wenshu in need_wenshus:
                for key in need_wenshus_common_keys_dict[need_wenshu]:
                    if "诊断" in key:
                        res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_list[need_wenshu], list(key))
                        break
            return res_str
            ...
            # 患者所需文书（总），以下根据规则筛选文书
            need_wenshus = need_wenshus_common_keys_dict.keys()
            # 将患者文书根据时间排序（最近的文书在最前面）
            need_wenshus = sorted(need_wenshus, key=lambda x: datetime.strptime(x["时间"], "%Y-%m-%d %H:%M:%S"))
            # 返回患者基本信息溯源内容
            res_str = ""
            ...
            for need_wenshu in need_wenshus:
                for key in need_wenshus_common_keys_dict[need_wenshu]:
                    if len(wenshu_list[need_wenshu]["内容"][key]) != 0:  # 获取到数据
                        res_str = transfer_wenshu_to_str(wenshu_list[need_wenshu], list(key))
                        break
            return res_str
            ...
            if "主任医师首次查房记录" in need_wenshus:
                res_str = transfer_wenshu_to_str(wenshu_list["主任医师首次查房记录"], ["补充病史与体征"]) 
            if "主治医师首次查房记录" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_list["主治医师首次查房记录"], ["TM员工名称TM 主治医生查房"])
            elif "主治医师日常查房记录" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_list["主治医师日常查房记录"], ["TM员工名称TM 主治医生查房"]) 
            elif "日常病程记录" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_list["日常病程记录"], ["TM员工名称TM 住院医师查房"])
            elif "入院记录" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_list["入院记录"], ["辅助检查"])
            ...
            if "首次病程记录" in need_wenshus:
                res_str = transfer_wenshu_to_str(wenshu_list["首次病程记录"], ["诊疗计划", "病例特点"]) 
            if "主治医师首次查房记录" in need_wenshus:
                res_str = res_str + "\n"+ transfer_wenshu_to_str(wenshu_list["主治医师首次查房记录"], ["诊疗计划"])
            elif "主任医师首次查房记录" in need_wenshus:
                res_str = transfer_wenshu_to_str(wenshu_list["主任医师首次查房记录"], ["对病情的分析", "诊疗意见"])
            if "化疗前知情同意书" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_list["化疗前知情同意书"], ["诊断"])
            # 获取治疗信息
            if "主治医师首次查房记录" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_list["主治医师首次查房记录"], ["TM员工名称TM 主治医生查房", "诊断", "诊断依据及鉴别诊断的分析"])
            elif "主任医师首次查房记录" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_list["主任医师首次查房记录"], ["TM员工名称TM 主任医生查房", "对病情的分析", "诊疗意见"])
            ...
            if "主治医师日常查房记录" in need_wenshus:
                ...
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_list["主任医师日常查房记录"], ["诊疗意见"])
            # 获得相关诊断
            if "主治医师首次查房记录" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_list["主治医师首次查房记录"], ["诊断","诊断依据与鉴别诊断的分析"])
            elif "化疗前知情同意书" in need_wenshus:
                res_str =  res_str + "\n" + transfer_wenshu_to_str(wenshu_list["化疗前知情同意书"], ["诊断"])
            elif "转入科记录" in need_wenshus:
                res_str =  res_str + "\n" + transfer_wenshu_to_str(wenshu_list["转入科记录"], ["目前诊断"])

            if "日常病程记录" in need_wenshus:
                res_str = res_str + "\n" +  transfer_wenshu_to_str(wenshu_list["日常病程记录"], ["TM员工名称TM 住院医师查房"])
            elif "阶段小结" in need_wenshus:
                res_str =  res_str + "\n" + transfer_wenshu_to_str(wenshu_list["阶段小结"], ["诊疗计划"])
            elif "术后第三日记录" in need_wenshus:
                res_str =  res_str + "\n" + transfer_wenshu_to_str(wenshu_list["术后第三日记录"], ["TM员工名称TM 住院医师查房"])
            elif "术后第二日记录" in need_wenshus:
                res_str =  res_str + "\n" + transfer_wenshu_to_str(wenshu_list["术后第二日记录"], ["TM员工名称TM 住院医师查房"])
            elif "术后第一日记录" in need_wenshus:
                res_str =  res_str + "\n" + transfer_wenshu_to_str(wenshu_list["术后第一日记录"], ["TM员工名称TM 住院医师查房"])
...
def build_data(ori_index,ori_data,data_maps,data_lengths,keshi):
    ...
    ** 入院时间返回3个值 **
    ...
    # ||全部检查|| ****因为异常的无法判断全，所以全部丢进去，最早计划找出异常的检查(存在随访/随诊)，减少输入长度****！！！
    suifang_jiancha_str = ''
    ...
    constants['全部检查'] = suifang_jiancha_str
    ...
    for key,value in data_maps.items():
            ...
            for search_wenshu in need_wenshus:
                ...
                if search_wenshu in pipei_name:
                    ...
                    if len(all_common_keys) != 0:                                            # +
                        need_wenshus_common_keys_dict[search_wenshu] = all_common_keys       # +

        #     # 当前文书的str
        #     if len(all_common_keys) == 0:
        #         continue
        #     tmp_wenshu_str = transfer_wenshu_to_str(wenshu, all_common_keys).strip() #！！！替换
        #     # 加入str中
        #     wenshu_str = wenshu_str.strip() + '\n\n' + tmp_wenshu_str   #！！！替换
        # wenshu_str = wenshu_str.strip()
        ...
        # keshi 科室名
        # wenshu_list 患者所有文书数据
        # need_wenshus_common_keys_dict 所有需要的文书的名称-所需字段字典
        wenshu_str = get_all_wenshu_keys_to_str(keshi, key, wenshu_list, need_wenshus_common_keys_dict).strip() # +
...
ruxianwaike_data_maps = {
    ...
    '病程与治疗情况':{
        ...
        'source_keys':{
            ...
            '文书':['化疗前知情同意书---诊断','首次病程记录---初步诊断','首次病程记录---诊断依据','首次病程记录---鉴别诊断','首次病程记录---诊疗计划','查房记录---诊疗意见','查房记录---补充病史与体征','查房记录---TM员工名称TM 主任医生查房','查房记录---TM员工名称TM 主治医生查房','查房记录----对病情的分析','查房记录----诊断依据与鉴别诊断的分析','查房记录---诊疗计划','日常查房记录','日常病程记录','手术记录单','术后首次病程记录','术后第一日记录','最后一日查房/病程记录','镜','阶段小结','转入科记录','辅检回报','死亡记录','死亡病历讨论','24小时内入出院记录---入院情况'],
        }
    },
    '出院后用药建议':{
        'output_keys':{
        ...
        'source_keys':{
            ...
            '文书':['入院告知书---患者信息','查房记录---诊断','入院记录---主治医师48小时诊断','新会诊记录---目前诊断','术后首次病程记录---术中诊断','最后一日查房/病程记录','阶段小结','转入科记录','辅检回报','死亡记录','死亡病历讨论','24小时内入出院记录---入院情况','化疗前知情同意书---诊断'],
        }
    },
    '出院时情况':{
        'output_keys':{
        ...
        'source_keys':{
            ...
            '文书':['查房记录---诊断','入院记录---主治医师48小时诊断','新会诊记录---目前诊断','术后','入院告知书---患者信息','日常查房记录','日常病程记录','最后一日查房/病程记录','阶段小结','转入科记录','辅检回报','首次病程记录---病例特点','入院记录---体格检查','入院记录---专科情况','死亡记录','死亡病历讨论','查房记录---TM员工名称TM 主任医生查房','查房记录---TM员工名称TM 主治医生查房','新会诊记录---病史及体检摘要','24小时内入出院记录---入院情况','新入院评估单---三、生理状况评估','化疗前知情同意书---诊断'],
        }
    },
...
zhongliuke_data_maps = {
    ...
    '出院诊断':{
        ...
        'source_keys':{
            '字段':['科室类别','全部诊断','文书','病理报告','未出结果病理报告','最后一个在院评估单'],
            '文书':['入院记录---初步诊断','入院记录---现病史','入院记录---主诉','入院记录---主治医师48小时诊断','查房记录---诊断','新会诊记录---目前诊断','术后首次病程记录---术中诊断','首次病程记录---初步诊断','首次病程记录---诊断依据','首次病程记录---鉴别诊断','首次病程记录---诊疗计划','主治医师查房记录---诊断','主治医师查房记录---诊断依据','主治医师查房记录---鉴别诊断','主治医师查房记录---诊疗计划','主任医师查房记录---对病情的分析','主任医师查房记录---诊疗意见','术前小结---术前诊断','术前小结---手术指征','最后一日查房/病程记录','镜','阶段小结','手术记录单','术后首次病程记录','查房记录---诊断依据与鉴别诊断的分析','术后第一日记录','24小时内入出院记录---入院情况','24小时内入出院记录---入院诊断','入院记录---辅助检查','首次病程记录---病理特点','查房记录---TM员工名称TM 主治医生查房','查房记录---TM员工名称TM 主任医生查房','转入科记录','新会诊记录---会诊意见','新会诊记录---会诊目的','日常病程记录','辅检回报','疑难病例讨论记录---讨论意见','疑难病例讨论记录---主持人总结','死亡记录','死亡病历讨论'],
        }
    },
    '病程与治疗情况':{
        'output_keys':{
    ...
    '出院后用药建议':{
        ...
        'source_keys':{
            '字段':['科室类别','全部诊断','文书','出院带药','异常检验指标','全部检查','未出检查结果报告','未出结果病理报告','最后一个在院评估单'],
            '文书':['查房记录---诊断','入院记录---主治医师48小时诊断','新会诊记录---目前诊断','术后首次病程记录---术中诊断','主治医师查房记录---诊疗计划','最后一日查房/病程记录','阶段小结','转入科记录','辅检回报','死亡记录','死亡病历讨论','24小时内入出院记录---入院情况','化疗前知情同意书---诊断'],
        }
    },
    '出院时情况':{
        'output_keys':{
        ...
        'source_keys':{
            '字段':['科室类别','文书','最后一个在院评估单'],
            '文书':['入院记录---现病史','24小时内入出院记录---入院情况','查房记录---诊断','入院记录---主治医师48小时诊断','新会诊记录---目前诊断','术后','日常查房记录','日常病程记录','最后一日查房/病程记录','阶段小结','转入科记录','辅检回报','首次病程记录---病例特点','入院记录---体格检查','入院记录---专科情况','死亡记录','死亡病历讨论','查房记录---TM员工名称TM 主任医生查房','查房记录---TM员工名称TM 主治医生查房','新会诊记录---病史及体检摘要','24小时内入出院记录---入院情况','新入院评估单---三、生理状况评估','化疗前知情同意书---诊断'],
        }
    },
...
xiaoerke_data_maps = {
    ...
    '患者基本信息':{
        ...
        'source_keys':{
            ...
            '文书':['入院告知书---患者信息','告未成年患者监护人书---患者信息','患者授权委托书---患者信息','新入院评估单---三、生理状况评估','入院记录---体格检查','入院记录---初步诊断','新会诊记录---目前诊断','术前小结---术前诊断','首次病程记录---初步诊断','术前小结---术前诊断','阶段小结---入院诊断','转入科记录---入院诊断','死亡记录---入院诊断', '死亡病历讨论'], 
        }
    },
    '出院诊断':{
        'output_keys':{
        ...
        'source_keys':{
            '字段':['科室类别','出院相关诊断','文书','病理报告','最后一个在院评估单'], 
            '文书':['术后首次病程记录---术中诊断', '术前小结---术前诊断','术前小结---手术指征','最后一日查房/病程记录','阶段小结---目前诊断','手术记录单','查房记录---诊断','查房记录---TM员工名称TM 主治医生查房','查房记录---TM员工名称TM 主任医生查房','转入科记录---目前诊断','新会诊记录---目前诊断','日常病程记录','辅检回报','疑难病例讨论记录---讨论意见','疑难病例讨论记录---主持人总结','PUCAI','PCDAI','镜','死亡记录','死亡病历讨论'],
        }
    },
    '入院时简要病史':{
        'output_keys':{
        ...
        'source_keys':{
            '字段':['科室类别','文书'],
            '文书':['入院记录---现病史', '首次病程记录---病例特点','24小时内入出院记录---入院情况','阶段小结---入院情况','转入科记录---入院情况','死亡记录---入院情况'], # '死亡病历讨论'
        }
    },
    '体检摘要':{
        'output_keys':{
        ...
        'source_keys':{
            '字段':['科室类别','文书'],
            '文书':['入院记录---专科情况','首次病程记录---病例特点','24小时内入出院记录---入院情况','转入科记录---目前情况'], # '死亡病历讨论'
        }
    },
    '住院期间医疗情况':{
        'output_keys':{
    ...
    '病程与治疗情况':{
        ...
        'source_keys':{
            '字段':['科室类别','文书','检查','检验','未出检查结果报告','病理报告','未出结果病理报告','最后一个在院评估单'],
            '文书':['化疗前知情同意书---诊断','首次病程记录---初步诊断','首次病程记录---诊断依据','首次病程记录---鉴别诊断','首次病程记录---诊疗计划','查房记录---诊疗意见','查房记录---补充病史与体征','查房记录---TM员工名称TM 主任医生查房','查房记录---TM员工名称TM 主治医生查房','查房记录----对病情的分析','查房记录----诊断依据与鉴别诊断的分析','查房记录---诊疗计划','手术记录单','术后首次病程记录','术后第一日记录','日常查房记录','日常病程记录','最后一日查房/病程记录','镜','阶段小结','转入科记录','辅检回报','死亡记录','死亡病历讨论','24小时内入出院记录---入院情况'],
        }
    },
    '出院时情况':{
        ...
        'source_keys':{
            '字段':['科室类别','文书','最后一个在院评估单'],
            '文书':['查房记录---诊断','入院记录---主治医师48小时诊断','新会诊记录---目前诊断','术后','查房记录---对病情的分析','查房记录---补充病史与体征','查房记录---TM员工名称TM 主任医生查房','查房记录---TM员工名称TM 主治医生查房','查房记录---主治医生查房','日常查房记录','日常病程记录','最后一日查房/病程记录','阶段小结','转入科记录','辅检回报','首次病程记录---病例特点','入院记录---体格检查','入院记录---专科情况','死亡记录','死亡病历讨论','查房记录---TM员工名称TM 主任医生查房','查房记录---TM员工名称TM 主治医生查房','新会诊记录---病史及体检摘要','24小时内入出院记录---入院情况','新入院评估单---三、生理状况评估','化疗前知情同意书---诊断'],
        }
    },
    '出院后用药建议':{
        'output_keys':{
        ...
        'source_keys':{
            ...
            '文书':['查房记录---诊断','术后首次病程记录---术中诊断','最后一日查房/病程记录','阶段小结---诊疗计划','化疗前知情同意书---诊断','转入科记录','辅检回报','死亡记录','死亡病历讨论'],
        }
    },
...
shenjingneike_data_maps = {
    ...
    '病程与治疗情况':{
        ...
        'source_keys':{
            '字段':['科室类别','文书','检查','未出检查结果报告','病理报告','未出结果病理报告','最后一个在院评估单'],
            '文书':['化疗前知情同意书---诊断','首次病程记录---初步诊断','首次病程记录---诊断依据','首次病程记录---鉴别诊断','首次病程记录---诊疗计划','查房记录---诊疗意见','查房记录---补充病史与体征','查房记录---TM员工名称TM 主任医生查房','查房记录---TM员工名称TM 主治医生查房','查房记录----对病情的分析','查房记录----诊断依据与鉴别诊断的分析','查房记录---诊疗计划','手术记录单','术后首次病程记录','术后第一日记录','日常查房记录','日常病程记录','最后一日查房/病程记录','镜','阶段小结','转入科记录','辅检回报','死亡记录','死亡病历讨论','24小时内入出院记录---入院情况'],
        }
    },
    '出院后用药建议':{
