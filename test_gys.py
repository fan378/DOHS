...
def upload_file(file):
    ...
    if which_model == 'ruxianwaike_singlecuda' or which_model == 'ruxianwaike':
        ...
        ############################################################################
        # 自动判断科室 未写完
        # doctor_content = { zylsh : json_datas[zylsh]['出院小结']['内容'] }
        # possible_keshi = doctor_content['基本信息']['科别']
...
def generate_file():
    ...
    if which_model == 'ruxianwaike_singlecuda' or which_model == 'ruxianwaike':
        ...
        ins_datas_v2024_0324 = get_instructions_v2024_0324(csv_data_path,exact_ins_out_dir,keshi)
    ...
    if which_model == 'ruxianwaike_singlecuda': # 单卡乳腺外科推理
        ...
        postprocess(zylsh,read_dir,now_out_dir,now_out_data_dir)
    # 存储测试用例
...
# 删除案例
def delete_patient(keshi,key_id):
    keshi = 'ruxianwaike'
    processed_out_dir = global_variable.get_value('processed_out_dir')
    ins_out_dir = global_variable.get_value('ins_out_dir')
...
if __name__ == '__main__':
    ...
    with gr.Blocks(css="custom.css",head=head) as demo:                
        ...
        with gr.Row(equal_height=True):
            ...
            table_colomn_1 = gr.Dropdown(choices=chinese_keshi_list[0],label='病人',allow_custom_value=False,
                                         container=False,show_label=False,min_width=60)
    ...
    demo.queue().launch(show_api= False,share=False, server_name="0.0.0.0", server_port=7862, inbrowser=False)
