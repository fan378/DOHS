...
def process_suifang(ori_datas, ori_source):
    ...
    for item in jiancha_list:
        flag, history = model.chat(tokenizer, "请根据检查结果中的‘图像分析’，判断下面的检查结果是否需要随访，请['是'，'否']中选择一个输出，不要输出其他内容。\n "+item, history=[] ,temperature=0.01)
        # print(flag)
        ...
        if '是' in flag or flag=='需要随访。':
            suifang_keshi,history= model.chat(tokenizer,"你是一位专业医生，请根据下面的检查给出对应的随访科室，请直接输出建议随访科室。\n"+item, history=[] ,temperature=0.01)
            # 请从下面的科室列表中选择随访科室:[‘呼吸科’,‘乳腺外科’,‘胰腺外科’,‘消化科’,‘高血压科’,‘骨科’,‘心内科’,‘妇科’,‘血液科’,‘肝胆外科’]
            # print(suifang_keshi)
            jiancha_summary,history= model.chat(tokenizer,"你是一位专业医生，请将下面的检查进行总结，请直接输出总结出的图像异常的检查结果，精炼语句，将输出控制在一句话，不要提出建议\n"+item, history=[] ,temperature=0.01)
    ...
    for i in suifang_dic:
        jiancha_suggestion=suifang_dic[i][:-1]
    ...
    # print("患者"+jiancha_suggestion+keshi_suggestion)
...
def get_history_suifang(past_history, suifang_keshi_dict)

# 根据既往史得到既往史随访建议
def get_jiwangshi_suifang(past_history, suifang_keshi_dict):
            ...
            if '否认' not in sub_item and sub_item:  # 确保不是空字符串
                ...
                for final_item in final_items:
                    ...
                    if '否认' not in final_item and final_item:  # 确保不是空字符串
                        ...
                        disease_sentences = final_item.split("，")
...
def correct_bingshi(wenshu_list,ori_datas):
    ...
    for wenshu in wenshu_list:
        if '入院记录' in wenshu['文书名']:
            if '既往史' in wenshu['内容']:
                past_history = past_history +"\t"+ wenshu['内容']['既往史']
        if '新入院评估单' == wenshu['文书名']:
            if '一、基本信息' in wenshu['内容']:
                past_history = past_history + "\t"+wenshu['内容']['一、基本信息'][wenshu['内容']['一、基本信息'].find('特殊既往史:'):wenshu['内容']['一、基本信息'].find('既往不安全事件发生史:')]
    
    ori_datas['出院后用药建议'] = ori_datas['出院后用药建议'] + get_history_suifang(past_history, suifang_keshi_dict)
    # ori_datas['出院后用药建议'] = ori_datas['出院后用药建议'] + ' ' + get_jiwangshi_suifang(past_history, suifang_keshi_dict)
