    ...
    if keshi == "xiaoerke":
        if ins_name == "患者基本信息":
            ...
            if "术后首次病程记录" in need_wenshus:
                ...
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_dict["手术记录单"], ["术中诊断"])
            elif "术前小结" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_dict["术前小结"], ["术前诊断"])
            ...
            for need_wenshu in need_wenshus:
                if flag is False:
                    for key in need_wenshus_common_keys_dict[need_wenshu]:
                        if key in wenshu_dict[need_wenshu]["内容"].keys():
            ...
            for need_wenshu in need_wenshus:
                if flag is False:
                    for key in need_wenshus_common_keys_dict[need_wenshu][:2]:
                        if key in wenshu_dict[need_wenshu]["内容"].keys():
                            if len(wenshu_dict[need_wenshu]["内容"][key]) != 0:  # 获取到数据
                                res_str = transfer_wenshu_to_str(wenshu_dict[need_wenshu], [key])
                                flag = True
            ...
            if "首次病程记录" in need_wenshus:
                temp_wenshu = deepcopy(wenshu_dict["首次病程记录"])
                if '病例特点' in temp_wenshu["内容"].keys():
                    temp_str_list = temp_wenshu["内容"]["病例特点"].split("\n")
                    new_str = ""
                    for temp_str in temp_str_list:
                        if "查体" in temp_str:
                            new_str = new_str + "\n" + temp_str
                    temp_wenshu["内容"]["病例特点"] = new_str
                    res_str = transfer_wenshu_to_str(temp_wenshu, ["病例特点"])
...
ruxianwaike_data_maps = {
    ...
    "病程与治疗情况":{
        ...
        "source_keys":{
            "字段":["科室类别","文书","未出检查结果报告","病理报告","未出结果病理报告","最后一个在院评估单"],
            "文书":["术后首次病程记录","查房记录","新会诊记录","日常病程记录","术后第","最后一日查房/病程记录","24小时内入出院记录","化疗前知情同意书","首次病程记录"],
        }
    },
    "出院后用药建议":{
        "output_keys":{
...
zhongliuke_data_maps = {
    ...
    "出院诊断":{
        ...
        "source_keys":{
            ...
            "文书":["查房记录---诊断","新会诊记录---目前诊断","术后首次病程记录---术中诊断", "主治医师查房记录---诊断", "术前小结---术前诊断","术前小结---手术指征","最后一日查房/病程记录","镜","阶段小结","手术记录单---术中诊断","查房记录---诊断依据与鉴别诊断的分析","术后第一日记录","24小时内入出院记录---入院情况","24小时内入出院记录---入院诊断","入院记录---辅助检查","首次病程记录---病理特点","查房记录---TM员工名称TM 主治医生查房","查房记录---TM员工名称TM 主任医生查房","转入科记录---目前诊断","新会诊记录---会诊意见","新会诊记录---会诊目的","日常病程记录","辅检回报","疑难病例讨论记录---讨论意见","疑难病例讨论记录---主持人总结","死亡记录","死亡病历讨论"],
        }
    },
    "病程与治疗情况":{
        "output_keys":{
...
xiaoerke_data_maps = {
    ...
    "出院诊断":{
        ...
        "source_keys":{
            ...
            # "字段":["科室类别","文书"], 
            "文书":["术后首次病程记录---术中诊断", "术前小结---术前诊断","术前小结---手术指征","最后一日查房/病程记录","阶段小结---目前诊断","手术记录单","查房记录---诊断","查房记录---TM员工名称TM 主治医生查房","查房记录---TM员工名称TM 主任医生查房","转入科记录---目前诊断","新会诊记录---目前诊断","日常病程记录","辅检回报","疑难病例讨论记录---讨论意见","疑难病例讨论记录---主持人总结","死亡记录","死亡病历讨论"],#,"PUCAI","PCDAI","镜"
        }
    },
    "入院时简要病史":{
        "output_keys":{
    ...
    "体检摘要":{
        ...
        "source_keys":{
            "字段":["科室类别","文书"],
            "文书":["入院记录---专科情况","首次病程记录---病例特点","首次病程记录---诊断依据","术前讨论记录---手术指征","术前小结---手术指征","主治医师首次查房记录---TM员工名称TM 主治医师查房","24小时内入出院记录---入院情况","转入科记录---目前情况"], # "死亡病历讨论"
        }
    },
    "住院期间医疗情况":{
        "output_keys":{
        ...
        "source_keys":{
            "字段":["科室类别","检查","检验","日常病程辅检","文书"],
            # "字段":["科室类别","文书"],
            "文书":["日常查房记录","入院记录---辅助检查","首次病程记录---病例特点","首次病程记录---诊断依据","查房记录---补充病史与体征","查房记录---入院后辅助检查","查房记录---TM员工名称TM 主治医生查房","查房记录---TM员工名称TM 主任医生查房","24小时内入出院记录---入院情况","阶段小结","转入科记录","辅检回报","死亡记录","死亡病历讨论"],
        }
    },
    "病程与治疗情况":{
        "output_keys":{
        ...
        "source_keys":{
            "字段":["科室类别","文书","检查","检验","病理报告","最后一个在院评估单"],
            # "字段":["科室类别","文书"],
    ...
    "出院时情况":{
        ...
        "source_keys":{
            "字段":["科室类别","文书","最后一个在院评估单"],
            # "字段":["科室类别","文书"],
            "文书":["上级医师查房记录---诊断依据", "主治医师首次查房记录---诊断","查房记录---诊断","术后","查房记录---对病情的分析","查房记录---补充病史与体征","查房记录---TM员工名称TM 主任医生查房","查房记录---TM员工名称TM 主治医生查房","查房记录---主治医生查房","日常查房记录","日常病程记录","最后一日查房/病程记录","阶段小结---目前诊断","转入科记录---目前诊断","辅检回报","死亡记录","死亡病历讨论","新会诊记录---病史及体检摘要","化疗前知情同意书---诊断","首次病程记录---病例特点"],
        }
    },
    "出院后用药建议":{
        "output_keys":{
...
def get_instructions_v2024_0324(data_dir,out_dir,keshi,data_type=0): # datatype: 0是训练、1是测试
    if out_dir != "":
...
if __name__ == "__main__":
    ...
    zylsh = sys.argv[3]
    out_dir = sys.argv[4]
    data_type = int(sys.argv[5]) # 0是训练数据 1是测试数据 -1是单个病人
    ...
    if data_type == 0 or data_type==1: # 批量生成数据
        ...
        get_instructions_v2024_0324(csv_data_path,now_out_dir,keshi,data_type)
    else:
        for zylsh in zylshs:
