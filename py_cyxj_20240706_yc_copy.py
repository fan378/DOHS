...
# 拿到未出的病理 对病理时间做判断
def get_weichu_bingli_list(bingli_list,admission_time,discharge_time):
    '''
    返回 正常病理、未出病理
    '''
    ...
    for index,bingli in enumerate(bingli_list):
        try:
            ...
            ######################################################################
            elif report_time < admission_time: # yc添加 入院之前做的病理 通常和本科室不相关
                # 如果时间差超过30天
                if (admission_time - report_time).days >= 30:
                    skip_bingli_list.append(bingli)
                    # print('时间超过30天，跳过这个病理')
                    continue
            ######################################################################
    ...
    if keshi == "xiaoerke":
        ...
        if ins_name == "患者基本信息":
            ...
            if "入院告知书" in need_wenshus:
                ...
                res_str = transfer_wenshu_to_str(wenshu_dict['新入院评估单'], ['一、基本信息'])
            elif "乳腺中心入院记录" in need_wenshus:
                res_str = transfer_wenshu_to_str(wenshu_dict['乳腺中心入院记录'], ['患者一般情况'])

            # 获取体征信息
            if "新入院评估单" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_dict['新入院评估单'], ['三、生理状况评估']) # 由于['患者信息']字段明确，且入院告知书只有一个字段，也可need_wenshus_common_keys_dict['入院告知书']
            elif "乳腺中心入院记录" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_dict['乳腺中心入院记录'], ['体格检查', '初步判断'])
            ...
            if "乳腺中心术后首次病程记录" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_dict['乳腺中心术后首次病程记录'], ['术中诊断']) 
            elif "手术记录单" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_dict['手术记录单'], ['手术日期']) #手术日期字段中包含所有的手术记录单数据
            elif "术前小结" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_dict['术前小结'], ['术前诊断', '手术指征'])
            elif "乳腺中心首次病程记录" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_dict['乳腺中心首次病程记录'], ['鉴别诊断'])
            elif "阶段小结" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_dict['阶段小结'], ['目前诊断'])
            elif "转入科记录" in need_wenshus:
                res_str = res_str + "\n" + transfer_wenshu_to_str(wenshu_dict['转入科记录'], ['目前诊断'])
        ...
        # '文书':['术后首次病程记录---术中诊断', '术前小结---术前诊断','术前小结---手术指征','阶段小结---目前诊断','手术记录单','转入科记录---目前诊断','最后一日查房/病程记录','查房记录---诊断','查房记录---TM员工名称TM 主治医生查房','查房记录---TM员工名称TM 主任医生查房','新会诊记录---目前诊断','日常病程记录','辅检回报','疑难病例讨论记录---讨论意见','死亡记录','死亡病历讨论'],#,'PUCAI','PCDAI','镜'
            ...
            if last_discharge_record != "" and shoushujilu_name != "":
                # 主要手术+冰冻
                if '乳腺中心术后首次病程记录' in need_wenshus:
                    res_str = res_str + "\n"+ transfer_wenshu_to_str(wenshu_dict["乳腺中心术后首次病程记录"], ["手术时间","麻醉方式","手术方式","手术简要经过",'术后处理措施'])
            # 只有24小时内入出院记录
...
def build_data(ori_index,ori_data,data_maps,data_lengths,keshi):
    ...
    ** 入院时间返回3个值 **
    ...
    if ruyuan_find:
        # 入院时间
        admission_time = ruyuan_time
    else:
        admission_time = '1900-01-01'
    ##########################################################################
    ...
    if jianyan_str == '':
        jianyan_str = '无'
    constants['全部检验'] = jianyan_str.strip()
    ...
    # ||病理报告||
    bingli_list, weichu_bingli_list, skip_bingli_list = get_weichu_bingli_list(bingli_list,admission_time,last_update_time)
...
ruxianwaike_data_maps = {
    ...
    '患者基本信息':{
        ...
        'source_keys':{
            ...
            '文书':['入院告知书---患者信息','告未成年患者监护人书---患者信息','患者授权委托书---患者信息','新入院评估单---一、基本信息','新入院评估单---三、生理状况评估','乳腺中心入院记录---患者一般情况','乳腺中心入院记录---体格检查','乳腺中心入院记录---初步诊断','新会诊记录---目前诊断','乳腺中心术前小结---术前诊断','乳腺中心首次病程记录---初步诊断','阶段小结---入院诊断','转入科记录---入院诊断','死亡记录---入院诊断', '死亡病历讨论'], 
        }
    },
    '出院诊断':{
        'output_keys':{
        ...
        'source_keys':{
            ...
            '文书':['乳腺中心术后首次病程记录---术中诊断', '乳腺中心术前小结---术前诊断','乳腺中心术前小结---手术指征','阶段小结---目前诊断','手术记录单','转入科记录---目前诊断','最后一日查房/病程记录','查房记录---诊断','查房记录---TM员工名称TM 主治医生查房','查房记录---TM员工名称TM 主任医生查房','新会诊记录---目前诊断','日常病程记录','辅检回报','疑难病例讨论记录---讨论意见','死亡记录','死亡病历讨论'],#,'PUCAI','PCDAI','镜'
           }
    },
    '入院时简要病史':{
        'output_keys':{
        ...
        'source_keys':{
            '字段':['科室类别','文书'],
            '文书':['乳腺中心入院记录---现病史','乳腺中心术前讨论记录---病史回顾', '乳腺中心首次病程记录---病例特点','乳腺中心24小时出入院记录---入院情况','乳腺中心24小时内入出院记录---入院情况','阶段小结---入院情况','转入科记录---入院情况','疑难病例讨论记录---病史回顾','死亡记录---入院情况'], # ,'新入院评估单---一、基本信息'
        }
    },
    '体检摘要':{
        'output_keys':{
        ...
        'source_keys':{
            '字段':['科室类别','文书'],
            '文书':['乳腺中心入院记录---专科情况','乳腺中心术前讨论记录---病史回顾','乳腺中心首次病程记录---病例特点','乳腺中心24小时出入院记录---入院情况','乳腺中心首次病程记录---诊断依据','乳腺中心术前讨论记录---手术指征','术前小结---手术指征','主治医师首次查房记录---TM 员工名称TM 主治医师查房','乳腺中心24小时内入出院记录---入院情况','转入科记录---目前情况'],  #,'新入院评估单---一、基本信息'
        }
    },
    '住院期间医疗情况':{
        'output_keys':{
        ...
        'source_keys':{
            ...
            '文书':['日常查房记录','入院记录---辅助检查','乳腺中心首次病程记录---病例特点','乳腺中心首次病程记录---诊断依据','查房记录---补充病史与体征','查房记录---入院后辅助检查','乳腺中心24小时内入出院记录---入院情况','阶段小结','转入科记录','辅检回报','死亡记录','死亡病历讨论'],
        }
    },
    '病程与治疗情况':{
        'output_keys':{
        ...
        'source_keys':{
            '字段':['科室类别','文书','全部检查','简化过滤检验','病理报告','最后一个在院评估单'],
            # 简化过滤检验 简化最新检验 简化检验 
            '文书':['主任医师首次查房记录','主治医师首次查房记录', '化疗前知情同意书---诊断','乳腺中心首次病程记录---病例特点','乳腺中心首次病程记录---诊疗计划','查房记录---诊疗意见','查房记录---补充病史与体征','查房记录---TM员工名称TM 主任医生查房','查房记录---TM员工名称TM 主治医生查房','查房记录----对病情的分析','查房记录----诊断依据与鉴别诊断的分析','查房记录---诊疗计划','手术记录单','乳腺中心术后首次病程记录','乳腺中心术后第一日记录','日常查房记录','日常病程记录','镜','阶段小结','转入科记录','辅检回报','死亡记录','死亡病历讨论','乳腺中心24小时内入出院记录---入院情况'],
        }
    },
    '出院时情况':{
        'output_keys':{
...
neifenmi_data_maps = {
    ...
    '病程与治疗情况':{
        ...
        'source_keys':{
            '字段':['科室类别','文书','全部检查','简化过滤检验','未出检查结果报告','病理报告','未出结果病理报告','最后一个在院评估单'],
...
def get_instructions_v2024_0324(data_dir,out_dir,keshi,data_type=-1): # datatype: 0是训练、1是测试、-1是单个病人
    if out_dir != "":
