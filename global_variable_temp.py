...
def single_json_to_html(json_content,key_id,html_name='template'):
...
def read_html_with_key_id(key_id=key_id,prefix='演示示例'):
    ...
    if now_mode in ['ruxianwaike', 'ruxianwaike_singlecuda']:
        ...
        if prefix == '演示示例':
            ...
            if key_id == '19031800000446':
                file_name = f"./{model_generated_path}/{keshi}/{key_id}_copy/{key_id}_postprocessed.json"
    ...
    if os.path.exists(file_name):
        with open(file_name,'r',encoding='utf8') as f:
            content = json.load(f)  
        html_content = single_json_to_html(content,key_id,html_name = template_name)
        return html_content
    else:
...
def load_two_html(key_id=key_id):
    ...
    if now_mode == 'quankeshi':
        ...
        if key_id == '19031800000446':
            model_file_path = f"./{model_generated_path}/{keshi}/{key_id}_copy/{key_id}_postprocessed.json"
        # 溯源页面
    ...
    if os.path.exists(model_file_path) and os.path.exists(doctor_file_path):
        ...
        model_content = single_json_to_html(model_json,key_id,html_name='template_red')
        doctor_content = single_json_to_html(doctor_json,key_id)
    else:
        model_content = read_html_with_key_id(key_id)
        doctor_content = read_html_with_key_id(key_id,prefix='医生示例')
    return model_content,doctor_content,backtracking_html
