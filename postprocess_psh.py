...
def process_suifang(ori_datas, ori_source):
    ...
    for item in jiancha_list:
        ...
        flag, history = model.chat(tokenizer, "请判断下面的检查结果是否有异常，请从['是'，'否']中选择一个输出，不要输出其他内容。\n "+ jiancha_summary, history=[] ,temperature=0.01)
        if '明显异常' in jiancha_summary or  '未见异常' in jiancha_summary or '未发现异常' in jiancha_summary:
            flag='否'


        if '是' in flag or flag=='需要随访。':
            rjkeshi_list="，请从下面的科室列表中选择随访科室:[‘呼吸内科’,‘乳腺外科’,‘心内科’,‘胰腺外科’,‘消化科’,‘高血压科’,‘骨科’,‘妇科’,‘血液科’,‘耳鼻喉科’,‘甲状腺血管科’,‘神经内科’,‘神经外科’,‘肾脏内科’,‘内分泌科’,‘胃肠外科’,‘小儿科’]"
...
def conclude_suggestion(ori_datas)
    ...
    conclusion,history= model.chat(tokenizer,conclude+ori_datas['出院后用药建议'], history=[] ,temperature=0.01)
    print(conclusion)
